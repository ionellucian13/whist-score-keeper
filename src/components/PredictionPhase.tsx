  ...
  return (
    <div className="card bg-app-surface shadow-app-elevation-2 rounded-radius-xl overflow-hidden">
      <div className="p-6">
        <header className="mb-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <div className="inline-flex items-center justify-center p-2 mr-3 rounded-full bg-primary-surface text-primary-color">
              ...
              <div>
                ...
                <p className="text-app-text-secondary">Faza de predicții: {currentRound.cardsPerPlayer} mâini</p>
              </div>
            </div>
        ...
        {currentPlayer && (
          <div className="current-player-info mb-6 p-4 bg-primary-surface rounded-xl border border-primary-border fade-in">
            <div className="flex items-center">
              <div className="flex-shrink-0 mr-3">
                <div className="w-10 h-10 rounded-full animate-pulse" style={{ backgroundColor: currentPlayer.color }}></div>
              </div>
              <div>
                <p className="text-primary-color font-medium">Este rândul lui <span className="font-bold">{currentPlayer.name}</span> să parieze</p>
                <p className="text-sm text-app-text-tertiary">Selectează numărul de mâini pe care crezi că le vei câștiga</p>
              </div>
            </div>
          </div>
        )}
            ...
            return (
              <div 
                ...
                className={`player-prediction relative p-4 rounded-xl border transition-all duration-300 ${
                  isCurrentPlayer 
                    ? 'bg-primary-surface border-primary-color shadow-app-elevation-1 transform scale-102' 
                    : hasPredicted 
                      ? 'bg-app-surface-hover border-app-border' 
                      : 'bg-app-surface border-app-border'
                } ${isDealerPlayer ? 'border-l-4 border-l-warning-500' : ''}`}
              >
                <div className="flex items-center justify-between">
                  <div className="player-identity flex items-center flex-1">
                    ...
                    ></div>
                    <span className="font-medium text-app-text-primary">{player.name}</span>
                    {isDealerPlayer && (
                      <span className="dealer-badge ml-2 text-xs px-1.5 py-0.5 bg-warning-surface text-warning-color rounded-sm border border-warning-border font-medium">
                        Dealer
                      </span>
                    )}
                  </div>
                  ...
                  {hasPredicted && (
                    <div className="prediction-value flex items-center">
                      <div className="py-1 px-2.5 bg-success-surface text-success-color rounded-md font-medium">
                ...
                {isCurrentPlayer && (
                  <div className="bet-buttons mt-4 grid grid-cols-9 sm:grid-cols-11 gap-2 fade-in">
                    {getPossibleBets(player.id).map((betValue) => {
                      const isRestricted = isDealerPlayer && getRestrictedNumbersForDealer().includes(betValue);
                      return (
                        <button
                          key={betValue}
                          className={`bet-button w-8 h-8 flex items-center justify-center rounded-md transition-all hover:shadow-md focus:outline-none ${
                            isRestricted
                              ? 'bg-app-surface-hover text-app-text-disabled cursor-not-allowed'
                              : 'bg-primary-color text-white hover:bg-primary-hover active:bg-primary-active'
                          }`}
                          onClick={() => {
                            if (!isRestricted) {
